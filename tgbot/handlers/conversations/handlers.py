...
register_handler = ConversationHandler(
    ...
    states={
        states.GET_USERNAME: [
            MessageHandler(Filters.regex(r"\b\S{6}\b"), onboarding_handlers.register)
        ]
    },
...
share_code_handler = ConversationHandler(
    ...
    states={
        states.GET_USER_TO_SHARE: [
            MessageHandler(
                Filters.regex(r"@[\S]+\b"), food_tickets_handlers.command_share_code
            )
        ]
    },
    fallbacks=[CommandHandler("back", onboarding_handlers.command_wait)],
